...
namespace ERP.Controllers
{
    ...
    public class MMS_DM_TinhTrangController : ControllerBase
    {
            ...
            if (supportedTypes.Contains(fileExt))
            {
                ...
                using (ExcelPackage package = new ExcelPackage(ms))
                {
                    ...
                    for (int i = 2; i <= rowCount; i++)
                    {
                        ...
                        var info = new ImportMMS_DM_TinhTrang();
        ...
        private int DuplicateSoKhungs(List<ImportMMS_DM_TinhTrang> lst)
        {
            return lst.GroupBy(p => new { p.Name }).Where(p => p.Count() > 1).Count();
        }

        [HttpPost("Save_Import")]
        public ActionResult Post_SaveImport(List<ImportMMS_DM_TinhTrang> DH)
        {
            lock (Commons.LockObjectState)
            {
                ...
                foreach (var item in DH)
                {
                    ...
                    var exit = uow.DM_TinhTrangs.GetSingle(x => !x.IsDeleted && x.Name.ToLower() == item.Name.ToLower());
                    if (exit == null)
                    {
